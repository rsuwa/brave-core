...
def main():
    ...
    # Runs after format so any leftover blank-line cosmetics are the
    # caller's call to re-format if they want.
    ...
    # Brave's style conventions (clang-format etc.).  `npm run format` walks
    # up from cwd to find brave/package.json -- repository.py already
    # enforces that we're inside the brave-core git tree, so this resolves.
