...
constexpr char kToolName[] = "semantic_history_search";
...
base::DictValue ScoredUrlRowToDict(const history_embeddings::ScoredUrlRow& row,
  ...
  const int passages_size = row.passages_embeddings.passages.passages_size();
  if (passages_size > 0) {
    if (include_all_passages) {
      ...
      for (size_t i : indices) {
        passages.Append(row.passages_embeddings.passages.passages(i));
      }
...
HistorySearchTool::RequiresUserInteractionBeforeHandling(
  ...
  // The search runs on locally-stored history that the user has already
  // opted into via the brave-history-embeddings flag; no remote send.
  return false;
}
...
void HistorySearchTool::OnSearchResult(
  ...
  // Surface the URLs in our results as a trusted-links artifact so the
  ...
  if (!result.scored_url_rows.empty()) {
    ...
    if (!links.empty()) {
      ...
      artifacts.push_back(mojom::ToolArtifact::New(
          /*id=*/std::nullopt, mojom::kTrustedLinksArtifactType,
          std::move(links_json)));
    }
  }
