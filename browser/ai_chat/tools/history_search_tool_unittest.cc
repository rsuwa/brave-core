...
void AddPassage(history_embeddings::ScoredUrlRow& row,
  ...
  row.passages_embeddings.embeddings.emplace_back(std::vector<float>{0.0f},
                                                  word_count);
...
TEST(HistorySearchToolJsonTest, EmptyResultProducesEmptyResultsArray) {
  ...
  EXPECT_EQ(
      ...
      base::test::ParseJsonDict(R"({"query": "kittens", "results": []})"));
}
...
TEST(HistorySearchToolJsonTest, ResultsAreSerialized) {
  ...
  EXPECT_EQ(base::test::ParseJsonDict(json),
            base::test::ParseJsonDict(base::StrCat({R"({
              "query": "article",
              "results": [{
                "title": "Article title",
