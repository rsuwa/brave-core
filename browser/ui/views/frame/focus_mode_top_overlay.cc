...
bool FocusModeTopOverlay::ShouldReveal() const {
  ...
  return reveal_lock_count_ > 0 || hover_detector_->hovering() ||
         temporary_reveal_timer_.IsRunning() ||
         reveal_watcher_->reasons().HasAny(kForceRevealReasons) ||
         ShouldRevealForTransientChild();
}

bool FocusModeTopOverlay::ShouldRevealForTransientChild() const {
  if (!reveal_watcher_->reasons().Has(
          EdgeRevealWatcher::RevealReason::kVisibleTransientChild)) {
    return false;
  }
  return GetRevealFraction() == 1.0;
}
