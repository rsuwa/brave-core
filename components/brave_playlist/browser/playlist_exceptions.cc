...
constexpr char kPlaylistExceptionsFile[] = "playlist_exceptions.json";
...
PlaylistResolveRule& PlaylistResolveRule::operator=(const PlaylistResolveRule&) =
    default;

PlaylistResolveRule::PlaylistResolveRule(PlaylistResolveRule&&) noexcept =
    default;

PlaylistResolveRule& PlaylistResolveRule::operator=(PlaylistResolveRule&&)
    noexcept = default;

// static
PlaylistExceptions* PlaylistExceptions::GetInstance() {
  return base::Singleton<PlaylistExceptions>::get();
}

PlaylistExceptions::PlaylistExceptions() = default;

PlaylistExceptions::~PlaylistExceptions() = default;

void PlaylistExceptions::ResetForTesting() {
  rules_.clear();
  is_ready_ = false;
  component_path_.clear();
}

void PlaylistExceptions::OnComponentReady(const base::FilePath& component_dir) {
  ...
  base::ThreadPool::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(
          ...
          component_path_.AppendASCII(kPlaylistExceptionsFile)),
      base::BindOnce(&PlaylistExceptions::OnPlaylistExceptionsLoaded,
                     weak_factory_.GetWeakPtr()));
}

void PlaylistExceptions::OnPlaylistExceptionsLoaded(
...
bool PlaylistExceptions::CanResolvePageSrcLater(const GURL& url) const {
  if (!is_ready_) {
    return true;
  }
...
std::vector<std::string> PlaylistExceptions::ListPlaylistExceptions() const {
